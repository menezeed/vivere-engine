...
describe('parseNarrativeFallback — ambiguidade (caso real "Forte São Mateus")', () => {
  ...
  it('preserva o texto original (sem HTML) para auditoria', () => {
    const result = parseNarrativeFallback(html, { year: 2026, month: 6 });
    expect(result.rawText).toContain('Forte São Mateus');
  });

  it('não extrai venueName quando a frase não usa preposição reconhecível ("o Forte São Mateus recebe")', () => {
    // Decisão explícita do usuário: não ampliar a heurística de venue
    // para evitar capturar sujeitos de frase incorretamente.
    const result = parseNarrativeFallback(html, { year: 2026, month: 6 });
...
describe('parseNarrativeFallback — caminho de sucesso (1 data clara, 1 horário claro)', () => {
  ...
  it('usa confidence SINGLE_CLEAR quando data e horário foram extraídos com sucesso', () => {
    const result = parseNarrativeFallback(html, { year: 2026, month: 6 });
    expect(result.confidence).toBe(NARRATIVE_CONFIDENCE.SINGLE_CLEAR);
  });

  it('confidence desta camada é estruturalmente menor que qualquer confiança típica da camada 1', () => {
    const result = parseNarrativeFallback(html, { year: 2026, month: 6 });
    expect(result.confidence).toBeLessThan(0.7); // camada 1 tipicamente produz 0.85+ nos casos reais validados
  });

  it('não gera reviewReasons quando venue foi extraído com sucesso', () => {
    const result = parseNarrativeFallback(html, { year: 2026, month: 6 });
...
describe('parseNarrativeFallback — extração parcial sinalizada (data ok, venue não encontrado)', () => {
  it('marca review_reason venue_not_extracted_from_narrative sem descartar data/hora já extraídos', () => {
    ...
    expect(result.status).toBe('extracted'); // data e hora extraídos com sucesso
...
describe('parseNarrativeFallback — apenas data, sem horário', () => {
  it('usa confidence DATE_ONLY (mais baixa) quando não há horário no texto', () => {
    const html = '<p>Neste sábado (15), o grupo de teatro apresenta sua nova peça para a comunidade.</p>';
    const result = parseNarrativeFallback(html, { year: 2026, month: 8 });

    expect(result.status).toBe('extracted');
    expect(result.date).toBe('2026-08-15');
    expect(result.time).toBeNull();
    expect(result.confidence).toBe(NARRATIVE_CONFIDENCE.DATE_ONLY);
  });
});

describe('parseNarrativeFallback — múltiplos horários ambíguos', () => {
  it('marca como ambíguo quando há mais de um horário mencionado sem associação clara', () => {
    const html =
      '<p>Neste domingo (5), a programação tem início às 14h, com uma roda de conversa, ' +
      'e às 17h, com apresentação musical.</p>';
    const result = parseNarrativeFallback(html, { year: 2026, month: 9 });

    expect(result.status).toBe('ambiguous');
    expect(result.ambiguityReason).toContain('múltiplos horários');
  });
});
...
describe('parseNarrativeFallback — nenhum sinal de data', () => {
  it('retorna not_found quando o texto não contém nenhuma data candidata', () => {
    ...
    expect(result.candidateDates).toHaveLength(0);
  });
});
