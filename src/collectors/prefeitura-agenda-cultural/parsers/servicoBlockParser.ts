...
/**
 * Parser de bloco SERVIÇO — camada 1 (alta confiança) do extrator de
 * duas camadas. Procura o marcador literal "SERVIÇO:" no texto e
 * extrai sub-blocos rotulados (Dia/Data/Horário/Local) que vêm depois.
 *
 * Decisão de design confirmada: 1 post pode gerar N eventos quando o
 * bloco SERVIÇO contém múltiplos sub-blocos (caso real observado:
 * "Arraiá da Praça da Bandeira" + "Arraiá do Peró" no mesmo post).
 * Cada sub-bloco é delimitado por uma linha em negrito que NÃO é um
 * dos rótulos conhecidos (Dia:, Horário:, Local:) — essa linha é o
 * título do sub-evento.
 *
 ...
 * recorrência do pipeline (estágio 03), não para o Collector.
 */

export type ServicoReviewReason = 'schedule_grouping_detected';

export interface ServicoSubEvent {
  title: string;
  date: string | null;
  time: string | null;
  endTime: string | null;
  ...
  reviewReason: ServicoReviewReason | null;
}
...
export interface ServicoBlockParseResult {
  found: boolean;
  subEvents: ServicoSubEvent[];
}
...
function splitServicoHtmlIntoSubBlocks(servicoHtml: string): string[] {
...
function parseSubBlock(blockHtml: string): ServicoSubEvent {
 ...
 * WordPress REST API) e a data de publicação (usada apenas como
 * referência de ano/mês se algum sub-bloco usar dia numérico isolado
 * — caso raro neste parser, mais comum no narrativeFallbackParser).
 */
export function parseServicoBlock(
  htmlContent: string,
  postPublishedAt: { year: number; month: number },
): ServicoBlockParseResult {
  const markerMatch = htmlContent.match(SERVICO_MARKER);
  ...
  const subBlocksHtml = splitServicoHtmlIntoSubBlocks(afterMarkerHtml);

  const subEvents = subBlocksHtml
    .map(parseSubBlock)
    .filter((ev) => ev.title.length > 0)
    // Defesa adicional: mesmo após a fusão de rótulos em
    // splitServicoHtmlIntoSubBlocks, um sub-evento cujo título
