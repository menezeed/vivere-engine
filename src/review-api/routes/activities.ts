...
export function activityRoutes(reviewService: ReviewService): Hono<Env> {
  ...
  router.get('/', async (c) => {
    ...
    try {
      const activities = await reviewService.listActivities(filter);
      return c.json({ data: activities, count: activities.length, filter });
  ...
  router.patch('/:id/approve', async (c) => {
    ...
    try {
      await reviewService.reviewActivity(c.req.param('id'), 'approve', user);
  ...
  router.patch('/:id/reject', async (c) => {
    const user = c.get('user');
    try {
      await reviewService.reviewActivity(c.req.param('id'), 'reject', user);
  ...
  router.patch('/:id/promote', async (c) => {
    const user = c.get('user');
    try {
      await reviewService.reviewActivity(c.req.param('id'), 'promote', user);
